...
describes what users get. Pick a format with `-o`, or let deckofcards choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
deckofcards <command> -o table   # aligned columns for reading
deckofcards <command> -o jsonl   # one JSON object per line, for piping
deckofcards <command> -o json    # a single JSON array
deckofcards <command> -o csv     # spreadsheet friendly
deckofcards <command> -o tsv     # tab-separated
deckofcards <command> -o url     # just the URL column
deckofcards <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
deckofcards <command> --fields id,title,url
```
...
```bash
deckofcards <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
deckofcards <command>            # a table, because this is a terminal
deckofcards <command> | wc -l    # JSONL, because this is a pipe
```
